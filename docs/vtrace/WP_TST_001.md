# WP-TST-001-R17 — closed corrective implementation-lineage amendment
...
identity, and ledger reference. R17 supersedes only those failed amendments and
authorizes one separately accepted R17 corrective entry and implementation
...
The R17 candidate commit is the exact commit containing these WP and Pulse 31
bytes. The R17 acceptance commit must have that exact R17 candidate commit as
its sole first parent; the R17 corrective-entry commit must be its later
direct governance successor; and the corrective implementation commit must be
the direct child of that R17 corrective entry. Accepted `WP-WS-001` exit
...
The R15 and R16 amendment commits are retained as failed governance and
authorize nothing. A later R17 acceptance and corrective entry authorize exactly one
corrective implementation commit as the direct non-merge child of the R17
...
2. `corrective_delta` is the exact projection from the accepted R17
...
3. `governance_delta` is the complete unprojected governance path sequence
   from failed implementation through R17 corrective entry. It binds the R15
   and R16 failed amendments, R17 candidate, future R17 acceptance, and future R17
...
```json
{
  ...
  "corrective_delta_unscoped_raw": ["<GIT-C>","diff","--raw","-z","--no-abbrev","--full-index","--no-ext-diff","--find-renames=100%","--diff-filter=AMDR","<corrective_entry_commit>","<corrective_implementation_commit>"],
  ...
  "governance_r17_candidate_raw": ["<GIT-C>","diff","--raw","-z","--no-abbrev","--full-index","--no-ext-diff","--find-renames=100%","--diff-filter=AMDR","be689ae15837b6f5ce0c24a89ee66a6b5939aa75","<r17_candidate_commit>","--","docs/vtrace/WP_TST_001.md","context/waves/2026-07-28-bastion-foundation/pulses/pulse-31-wp-tst-001-r17-corrective-lineage-candidate.md"],
  "governance_r17_acceptance_raw": ["<GIT-C>","diff","--raw","-z","--no-abbrev","--full-index","--no-ext-diff","--find-renames=100%","--diff-filter=AMDR","<r17_candidate_commit>","<r17_acceptance_commit>","--","context/waves/2026-07-28-bastion-foundation/pulses/pulse-32-wp-tst-001-r17-acceptance.md"],
  "r17_acceptance_commit_object": ["<GIT-C>","cat-file","commit","<r17_acceptance_commit>"],
  "governance_r17_entry_raw": ["<GIT-C>","diff","--raw","-z","--no-abbrev","--full-index","--no-ext-diff","--find-renames=100%","--diff-filter=AMDR","<r17_acceptance_commit>","<corrective_entry_commit>","--","context/waves/2026-07-28-bastion-foundation/pulses/pulse-33-wp-tst-001-r17-corrective-entry.md"]
}
```
...
subsequence is the complete R17-entry-to-child change and contains no extra
...
by `governance_r15_raw`; failed R16 has the two paths named by
`governance_r16_failed_raw`; the R17 candidate has the two paths named by
`governance_r17_candidate_raw`; acceptance has only Pulse 32; corrective entry
has only Pulse 33. Each governance observation must show exactly its stated
...
`be689ae15837b6f5ce0c24a89ee66a6b5939aa75 -> <r17_candidate_commit>`,
`<r17_candidate_commit> -> <r17_acceptance_commit>`, and
`<r17_acceptance_commit> -> <corrective_entry_commit>`, expanded to the full
...
`r17_acceptance_commit_object` must exit zero, retain complete raw stdout and
stderr/counts/hashes, and parse exactly one `parent` header whose value is the
bound R17 candidate commit. Its stdout SHA-256 and parsed parent are retained
in `AcceptanceBindingV3`; a merge, root, extra/missing parent, replacement,
abbreviation, malformed commit, or candidate mismatch rejects.
...
`A .../pulse-32-wp-tst-001-r17-acceptance.md`; and
`A .../pulse-33-wp-tst-001-r17-corrective-entry.md`, where each `.../` expands
only to `context/waves/2026-07-28-bastion-foundation/pulses/`. No alias is
retained in a row.

### 3.1 R17 lineage objects and schema-version closure

R17 replaces, rather than informally extending, every affected R14/R15/R16 record.
...
| `GovernanceDeltaBinding` / `test-gate-governance-delta.v2` | `schema,r15_failed_amendment_commit,r15_paths,r15_scoped,r15_unscoped,r16_failed_amendment_commit,r16_paths,r16_scoped,r16_unscoped,r17_candidate_commit,r17_candidate_paths,r17_candidate_scoped,r17_candidate_unscoped,r17_acceptance_commit,r17_acceptance_first_parent,r17_acceptance_paths,r17_acceptance_scoped,r17_acceptance_unscoped,corrective_entry_commit,corrective_entry_paths,corrective_entry_scoped,corrective_entry_unscoped,aggregate_digest`; path arrays are the exact inventories above; acceptance first parent is the exact R17 candidate commit and is verified from raw acceptance commit bytes; each scoped/unscoped member is a `GovernanceGitObservation`; each pair's stdout bytes/hash/count are equal; aggregate hashes `<command-id><TAB><observation-digest><LF>` for all ten observations in field order followed by `<commit><TAB><path><LF>` for every path in governance sequence; digest is last. |
...
| `FailedAmendmentBinding` / `test-gate-failed-amendment.v2` | `schema,wp_revision,commit,first_parent,wp_sha256,wp_blob,pulse_path,pulse_sha256,pulse_blob,review_id,review_digest,actionable_count,disposition,accepted,entered,authority_created,binding_digest`; exactly two bindings exist in revision order: R15 uses section 1 identities, review `AUDIT-WP-TST-001-R15-AMENDMENT-001`, count `5`; R16 uses section 1 identities, review `AUDIT-WP-TST-001-R16-AMENDMENT-001`, count `6`; both have disposition `failed-retained` and three false booleans; binding digest is last. |
| `CorrectiveLineageBinding` / `test-gate-corrective-lineage.v2` | `schema,wp_revision,wp_artifact_digest,r17_candidate_commit,acceptance_binding,corrective_entry_binding,original_r14_entry_commit,original_r14_entry_pulse_digest,failed_attempt,failed_amendments,governance_delta,complete_implementation_delta,corrective_delta,corrective_implementation_commit,corrective_implementation_tree,lineage_digest`; revision is `R17`; failed amendments are exactly `[R15,R16]`; positive WP/acceptance/entry values are R17 only; original R14 fields occur solely as complete-delta base custody; all nested digests recompute; lineage digest is last. |
| `AcceptanceBindingV3` / `test-gate-acceptance-binding.v3` | `schema,wp_revision,commit,first_parent,commit_object_sha256,commit_observation_digest,pulse_path,pulse_digest,subject_id,subject_digest,binding_digest`; revision `R17`, first parent exactly equals `r17_candidate_commit`; commit-object SHA hashes exact raw stdout and observation digest binds the exact `r17_acceptance_commit_object` argv/exits/streams plus parsed sole parent; path Pulse 32, subject `WP-TST-001-R17-ACCEPTANCE`, subject digest equals R17 WP digest; binding digest is last. |
| `CorrectiveEntryBindingV3` / `test-gate-corrective-entry-binding.v3` | `schema,wp_revision,commit,tree_digest,first_parent,pulse_path,pulse_digest,subject_id,acceptance_commit,acceptance_pulse_digest,binding_digest`; revision `R17`, first parent is R17 acceptance, path Pulse 33, subject `WP-TST-001-R17-CORRECTIVE-ENTRY`; binding digest is last. |
| `ImplementationBindingV3` / `test-gate-implementation-binding.v3` | `schema,commit,tree_digest,first_parent,allowed_paths,failed_attempt_binding_digest,failed_amendment_binding_digests,governance_delta_digest,complete_implementation_delta,corrective_delta,corrective_no_extra_observation,observed_preflight,binding_digest`; first parent equals corrective entry; allowed paths are exact 18; failed amendment digests are exactly R15 then R16; complete/corrective nested bindings validate independently; the no-extra observation is the exact unscoped R17-entry-to-child command and must have stdout byte-identical to corrective raw stdout but supplies no manifest row; binding digest is last. |
| `AuthorBindingV2` / `vtrace-author-binding.v2` | `schema,source_kind,source_ref,source_digest,wp_revision,lineage_digest,author_id,controller_id,binding_digest`; source kind is exactly `wp_r17_candidate|r17_acceptance_pulse|r17_corrective_entry_pulse|failed_r14_implementation|corrective_implementation_commit|mode_evidence|set_evidence`; R14/R15/R16 governance is not a positive registry source; failed kind must use failed-attempt custody and grants no authority; binding digest is last. |
| `IdentityRegistryV2` / `vtrace-identity-registry.v2` | `schema,bindings,candidate_author_ids,failed_author_ids,registry_digest`; bindings are source-kind/source-ref sorted unique `AuthorBindingV2`; candidate IDs project every author/controller ID having at least one positive nonfailed binding, even when the same ID also occurs in a failed binding; failed IDs project every failed-kind ID for visibility and independence collision checks. Failed bindings confer no authority. An ID is excluded from candidate IDs only when all of its bindings are failed-only; registry digest is last. |
...
R17 corrective evidence may close the technical conditions only by new results;
...
digest is last. Neither review nor either failed WP/pulse may appear
as a positive registry source, executable entry, implementation input, review
pass, or promotion claim.

Future Pulse 32 contains exactly one fenced `vtrace-author-custody.v2` block
with LF rows in this order: `subject=WP-TST-001-R17-ACCEPTANCE`,
`wp_revision=R17`, `wp_digest=<R17-WP-DIGEST>`,
`r17_candidate_commit=<R17-CANDIDATE-COMMIT>`,
`acceptance_first_parent=<R17-CANDIDATE-COMMIT>`,
...
```text
schema=corrective-lineage-intent.v2
wp_revision=R17
wp_digest=<R17-WP-DIGEST>
...
the brackets are not hashed. Pulse 33 contains exactly one fenced
`vtrace-author-custody.v2` block with rows `subject=WP-TST-001-R17-CORRECTIVE-ENTRY`,
`wp_revision=R17`, `wp_digest=<R17-WP-DIGEST>`,
`acceptance_commit=<R17-ACCEPTANCE-COMMIT>`,
`acceptance_pulse_digest=<PULSE-32-DIGEST>`,
`author_id=REV-TST-ENTRY-AUTHOR`,
`controller_id=REV-TST-GOVERNANCE-CONTROLLER`. Neither block contains or hashes
its own containing future commit. Their parsed fields populate only the v3
...
both ordered `failed_amendments[*].binding_digest` values, both delta digests, and
...
All subsequent references to R14 evidence schemas are replaced by the R17
...
R17 retains the exact R14 serial phase plan. Each closed `ExecutionPhase` has ordered
...
`wp_revision` is literal `R17`; `wp_digest` is the accepted R17 WP digest;
...
`test-gate-ledger.v3` record schema; its execution ID/version/run root bind those exact
records.
...
only the no-extra/subset assertion described in section 3. No implicit prefix,
...
| `wp_revision` | string literal `R17` |
| `wp_artifact_digest` | `DIGEST` of independently accepted R17 bytes |
...
| `acceptance_binding` | exact `AcceptanceBindingV3` equal to `lineage_binding.acceptance_binding` |
| `corrective_entry_binding` | exact `CorrectiveEntryBindingV3` equal to `lineage_binding.corrective_entry_binding` |
| `implementation_binding` | closed `ImplementationBindingV3` below and equal to the lineage corrective implementation/deltas/no-extra observation |
...
|---|---|
| `ExecutionOrigin` | exact keys `wp_revision,lineage_digest,execution_id,execution_ordinal,execution_evidence_version,implementation_commit,run_id,ledger_root,ledger_aggregate_digest`; revision/lineage equal R17 top level; values equal the implementation and ledger bindings; ordinal is independent of evidence version; it contains no evidence ID/version/path/digest and is byte-identical in every review successor |
...
| `AcceptanceBindingV3` | exact closed section 3.1 object; only R17 acceptance is positive and its sole first parent is the exact R17 candidate |
| `CorrectiveEntryBindingV3` | exact closed section 3.1 object; only R17 corrective entry is executable entry |
| `ImplementationBindingV3` | exact closed section 3.1 object; current commit/tree, R17 first parent, exact complete/corrective deltas, no-extra observation, governance and failed-attempt custody all validate |
...
| `AuthorBindingV2` | exact closed section 3.1 object; positive and failed source kinds, R17 revision, lineage digest, failed-only exclusion, and dual-bound-ID visibility are mandatory |
...
| `EvidenceBinding` | exact keys `wp_revision,lineage_digest,evidence_id,evidence_path,evidence_version,evidence_digest`; revision/lineage equal the enclosing R17 record; ID/path use the same mode and bound version formulas and digest hashes that immutable predecessor |

Identity extraction is literal and total. `wp_r17_candidate` hashes committed
`pulse-31-wp-tst-001-r17-corrective-lineage-candidate.md` bytes and extracts
...
`subject_digest=<wp_artifact_digest>`. `r17_acceptance_pulse` and
`r17_corrective_entry_pulse` similarly use future committed pulses 32 and 33,
subjects `WP-TST-001-R17-ACCEPTANCE` and
`WP-TST-001-R17-CORRECTIVE-ENTRY`, authors
...
`wp_revision=R17`, `lineage_digest=<lineage_binding.lineage_digest>`, and
...
are respectively the committed R17 candidate, R17 acceptance, R17 corrective
...
For R17 the kind-to-schema/digest-key map is exact:
...
assembly, or promotable under R17.
...
For both v2 publication records, `wp_revision` is literal `R17` and
`lineage_binding` is byte-identical to the enclosing evidence v12 object. Its
accepted R17 WP, corrective entry, failed attempt, governance delta, complete
...
`vtrace-review-auth.v2`, revision is literal `R17`, lineage digest equals the
...
cannot authenticate an R17 review.
...
WP ID is literal `WP-TST-001`; `wp_revision` is literal `R17`; WP digest,
...
unique union of the 16 selected mode registries: the shared R17 candidate,
...
All selected records have identical R17 WP/lineage/acceptance/corrective-entry/
...
The dedicated R17 negative suite retains every R14 and failed-R16 case and additionally rejects a reused/preexisting Job
...
R17 corrective-entry decision. The acceptance pulse binds the R17 artifact
digest, its exact R17 candidate first parent, accepted R14 custody, the retained
failed implementation, failed R15 and R16 amendments, exact governance
...
R17 WP/acceptance/corrective entry as its only positive governance identity,
and binds the original R14 entry solely as complete-delta base plus the failed
R14/R15/R16 custody. A clean isolated worktree starts at the R17 corrective-entry

# WP-TST-001-R18 — closed corrective implementation-lineage amendment
...
and left set-level dual-bound identity visibility ambiguous. R18 supersedes
only the failed amendments and authorizes one separately accepted R18 corrective entry and implementation
...
The R18 candidate commit is the exact commit containing these WP and Pulse 32
bytes. The R18 acceptance commit must have that exact R18 candidate commit as
its sole first parent; the R18 corrective-entry commit must be its later
direct governance successor; and the corrective implementation commit must be
the direct child of that R18 corrective entry. Accepted `WP-WS-001` exit
...
The R15, R16, and R17 amendment commits are retained as failed governance and
authorize nothing. A later R18 acceptance and corrective entry authorize exactly one
corrective implementation commit as the direct non-merge child of the R18
...
2. `corrective_delta` is the exact projection from the accepted R18
...
3. `governance_delta` is the complete unprojected governance path sequence
   from failed implementation through R18 corrective entry. It binds the R15,
   R16, and R17 failed amendments, R18 candidate, future R18 acceptance, and future R18
...
```json
{
  ...
  "governance_r18_candidate_raw": ["<GIT-C>","diff","--raw","-z","--no-abbrev","--full-index","--no-ext-diff","--find-renames=100%","--diff-filter=AMDR","e7eb6be0adcdb5afc069f5cd3e45cf62dbc31a6e","<r18_candidate_commit>","--","docs/vtrace/WP_TST_001.md","context/waves/2026-07-28-bastion-foundation/pulses/pulse-32-wp-tst-001-r18-corrective-lineage-candidate.md"],
  "governance_r18_acceptance_raw": ["<GIT-C>","diff","--raw","-z","--no-abbrev","--full-index","--no-ext-diff","--find-renames=100%","--diff-filter=AMDR","<r18_candidate_commit>","<r18_acceptance_commit>","--","context/waves/2026-07-28-bastion-foundation/pulses/pulse-33-wp-tst-001-r18-acceptance.md"],
  "r18_acceptance_commit_object": ["<GIT-C>","cat-file","commit","<r18_acceptance_commit>"],
  ...
  "governance_r18_entry_raw": ["<GIT-C>","diff","--raw","-z","--no-abbrev","--full-index","--no-ext-diff","--find-renames=100%","--diff-filter=AMDR","<r18_acceptance_commit>","<corrective_entry_commit>","--","context/waves/2026-07-28-bastion-foundation/pulses/pulse-34-wp-tst-001-r18-corrective-entry.md"]
}
```
...
subsequence is the complete R18-entry-to-child change and contains no extra
...
and the R18 candidate each have the two paths named by their exact arrays;
acceptance has only Pulse 33; corrective entry has only Pulse 34. Each
governance observation must show exactly its stated `M/A`, `M/A`, `M/A`,
`M/A`, `A`, or `A` path inventory, respectively, and the union is
...
`e7eb6be0adcdb5afc069f5cd3e45cf62dbc31a6e -> <r18_candidate_commit>`,
`<r18_candidate_commit> -> <r18_acceptance_commit>`, and
`<r18_acceptance_commit> -> <corrective_entry_commit>`, expanded to the full
...
`A .../pulse-33-wp-tst-001-r18-acceptance.md`; and
`A .../pulse-34-wp-tst-001-r18-corrective-entry.md`, where each `.../` expands
only to `context/waves/2026-07-28-bastion-foundation/pulses/`. No alias is
retained in a row.

### 3.1 R18 lineage objects and schema-version closure

R18 replaces, rather than informally extending, every affected R14/R15/R16/R17 record.
...
| `AcceptanceCommitObservation` / `test-gate-commit-parent-observation.v1` | the acceptance-role use of the one shared `CommitParentObservation` schema above, with no alternate keys or preimage; command ID is `r18_acceptance_commit_object`, commit is R18 acceptance, and first parent is the exact R18 candidate. Entry and implementation use that identical schema with only their formula-bound command/commit/tree/parent values changed. |
| `GovernanceDeltaBinding` / `test-gate-governance-delta.v3` | `schema,r15_failed_amendment_commit,r15_paths,r15_scoped,r15_unscoped,r16_failed_amendment_commit,r16_paths,r16_scoped,r16_unscoped,r17_failed_amendment_commit,r17_paths,r17_scoped,r17_unscoped,r18_candidate_commit,r18_candidate_paths,r18_candidate_scoped,r18_candidate_unscoped,r18_acceptance_commit,r18_acceptance_paths,r18_acceptance_scoped,r18_acceptance_unscoped,corrective_entry_commit,corrective_entry_paths,corrective_entry_scoped,corrective_entry_unscoped,aggregate_digest`; path arrays are exact; each unscoped observation uses no diff filter and must be byte-identical to its scoped peer; aggregate hashes all twelve observations then commit/path rows in field order; digest is last. |
...
| `FailedAmendmentBinding` / `test-gate-failed-amendment.v3` | same exact ordered keys/rules as v2; exactly three bindings exist in revision order R15,R16,R17 using section 1 identities, reviews `AUDIT-WP-TST-001-R15-AMENDMENT-001`, `...R16...`, `...R17...`, actionable counts `5,6,4`, disposition `failed-retained`, and three false booleans; binding digest is last. |
| `CorrectiveLineageBinding` / `test-gate-corrective-lineage.v3` | `schema,wp_revision,wp_artifact_digest,r18_candidate_commit,acceptance_binding,corrective_entry_binding,original_r14_entry_commit,original_r14_entry_pulse_digest,failed_attempt,failed_amendments,governance_delta,complete_implementation_delta,corrective_delta,corrective_implementation_commit,corrective_implementation_tree,lineage_digest`; revision `R18`; failed amendments exactly `[R15,R16,R17]`; positive WP/acceptance/entry values are R18 only; all nested observations/digests recompute; lineage digest is last. |
| `AcceptanceBindingV4` / `test-gate-acceptance-binding.v4` | `schema,wp_revision,commit,first_parent,commit_observation,pulse_path,pulse_digest,subject_id,subject_digest,binding_digest`; revision `R18`; observation is exact `AcceptanceCommitObservation`, its commit equals this commit and its sole first parent equals `r18_candidate_commit`; path Pulse 33, subject `WP-TST-001-R18-ACCEPTANCE`, subject digest equals R18 WP digest; binding digest is last. |
| `CorrectiveEntryBindingV4` / `test-gate-corrective-entry-binding.v4` | `schema,wp_revision,commit,tree_digest,first_parent,commit_observation,pulse_path,pulse_digest,subject_id,acceptance_commit,acceptance_pulse_digest,binding_digest`; revision `R18`; exact `CommitParentObservation` command ID `corrective_entry_commit_object` binds commit/tree and exactly one parent equal to acceptance; path Pulse 34, subject `WP-TST-001-R18-CORRECTIVE-ENTRY`; binding digest is last. |
| `ImplementationBindingV4` / `test-gate-implementation-binding.v4` | `schema,commit,tree_digest,first_parent,commit_observation,allowed_paths,failed_attempt_binding_digest,failed_amendment_binding_digests,governance_delta_digest,complete_implementation_delta,corrective_delta,corrective_no_extra_observation,observed_preflight,binding_digest`; exact `CommitParentObservation` command ID `corrective_implementation_commit_object` binds commit/tree and exactly one parent equal to corrective entry; failed amendment digests are R15,R16,R17; no-extra observation has no diff filter and must equal corrective raw stdout; binding digest is last. |
| `AuthorBindingV2` / `vtrace-author-binding.v2` | `schema,source_kind,source_ref,source_digest,wp_revision,lineage_digest,author_id,controller_id,binding_digest`; source kind is exactly `wp_r18_candidate|r18_acceptance_pulse|r18_corrective_entry_pulse|failed_r14_implementation|corrective_implementation_commit|mode_evidence|set_evidence`; R14/R15/R16/R17 governance is not a positive registry source; failed kind grants no authority; binding digest is last. |
...
R18 corrective evidence may close the technical conditions only by new results;
...
Future Pulse 33 contains exactly one fenced `vtrace-author-custody.v2` block
with LF rows in this order: `subject=WP-TST-001-R18-ACCEPTANCE`,
`wp_revision=R18`, `wp_digest=<R18-WP-DIGEST>`,
`r18_candidate_commit=<R18-CANDIDATE-COMMIT>`,
`acceptance_first_parent=<R18-CANDIDATE-COMMIT>`,
...
```text
schema=corrective-lineage-intent.v3
wp_revision=R18
wp_digest=<R18-WP-DIGEST>
...
the brackets are not hashed. Pulse 34 contains exactly one fenced
`vtrace-author-custody.v2` block with rows `subject=WP-TST-001-R18-CORRECTIVE-ENTRY`,
`wp_revision=R18`, `wp_digest=<R18-WP-DIGEST>`,
`acceptance_commit=<R18-ACCEPTANCE-COMMIT>`,
`acceptance_pulse_digest=<PULSE-33-DIGEST>`,
`author_id=REV-TST-ENTRY-AUTHOR`,
`controller_id=REV-TST-GOVERNANCE-CONTROLLER`. Neither block contains or hashes
its own containing future commit. Their parsed fields populate only the v4
...
All subsequent references to R14 evidence schemas are replaced by the R18
...
R18 retains the exact R14 serial phase plan. Each closed `ExecutionPhase` has ordered
...
`wp_revision` is literal `R18`; `wp_digest` is the accepted R18 WP digest;
...
| `wp_revision` | string literal `R18` |
| `wp_artifact_digest` | `DIGEST` of independently accepted R18 bytes |
...
| `acceptance_binding` | exact `AcceptanceBindingV4` equal to `lineage_binding.acceptance_binding` |
| `corrective_entry_binding` | exact `CorrectiveEntryBindingV4` equal to `lineage_binding.corrective_entry_binding` |
| `implementation_binding` | closed `ImplementationBindingV4` below and equal to the lineage corrective implementation/deltas/no-extra/commit-parent observations |
...
|---|---|
| `ExecutionOrigin` | exact keys `wp_revision,lineage_digest,execution_id,execution_ordinal,execution_evidence_version,implementation_commit,run_id,ledger_root,ledger_aggregate_digest`; revision/lineage equal R18 top level; values equal the implementation and ledger bindings; ordinal is independent of evidence version; it contains no evidence ID/version/path/digest and is byte-identical in every review successor |
...
| `AcceptanceBindingV4` | exact closed section 3.1 object; only R18 acceptance is positive and its retained commit observation proves the sole R18-candidate parent |
| `CorrectiveEntryBindingV4` | exact closed section 3.1 object; only R18 corrective entry is executable entry and its retained commit observation proves the acceptance parent |
| `ImplementationBindingV4` | exact closed section 3.1 object; current commit/tree, retained sole R18-entry parent, exact complete/corrective deltas, unfiltered no-extra observation, governance and failed custody all validate |
...
| `AuthorBindingV2` | exact closed section 3.1 object; positive and failed source kinds, R18 revision, lineage digest, failed-only exclusion, and dual-bound-ID visibility are mandatory |
...
| `EvidenceBinding` | exact keys `wp_revision,lineage_digest,evidence_id,evidence_path,evidence_version,evidence_digest`; revision/lineage equal the enclosing R18 record; ID/path use the same mode and bound version formulas and digest hashes that immutable predecessor |

Identity extraction is literal and total. `wp_r18_candidate` hashes committed
`pulse-32-wp-tst-001-r18-corrective-lineage-candidate.md` bytes and extracts
...
`subject_digest=<wp_artifact_digest>`. `r18_acceptance_pulse` and
`r18_corrective_entry_pulse` similarly use future committed pulses 33 and 34,
subjects `WP-TST-001-R18-ACCEPTANCE` and
`WP-TST-001-R18-CORRECTIVE-ENTRY`, authors
...
`wp_revision=R18`, `lineage_digest=<lineage_binding.lineage_digest>`, and
...
are respectively the committed R18 candidate, R18 acceptance, R18 corrective
...
For R18 the kind-to-schema/digest-key map is exact:
...
assembly, or promotable under R18.
...
For both v2 publication records, `wp_revision` is literal `R18` and
`lineage_binding` is byte-identical to the enclosing evidence v12 object. Its
accepted R18 WP, corrective entry, failed attempt, governance delta, complete
...
`vtrace-review-auth.v2`, revision is literal `R18`, lineage digest equals the
...
cannot authenticate an R18 review.
...
WP ID is literal `WP-TST-001`; `wp_revision` is literal `R18`; WP digest,
...
unique union of the 16 selected mode registries: the shared R18 candidate,
...
All selected records have identical R18 WP/lineage/acceptance/corrective-entry/
...
The dedicated R18 negative suite retains every R14 and failed-R16/R17 case and
...
R18 corrective-entry decision. The acceptance pulse binds the R18 artifact
digest, its exact R18 candidate first parent and complete commit observation,
accepted R14 custody, the retained failed
implementation, failed R15, R16, and R17 amendments, exact governance
...
R18 WP/acceptance/corrective entry as its only positive governance identity,
and binds the original R14 entry solely as complete-delta base plus the failed
R14/R15/R16/R17 custody. A clean isolated worktree starts at the R18 corrective-entry
